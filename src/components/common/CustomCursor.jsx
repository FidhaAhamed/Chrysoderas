...
const CustomCursor = () => {
    ...
    const [isHidden, setIsHidden] = useState(true); // Hide when mouse leaves window

    // Mouse coordinates
    const mouseX = useMotionValue(0);
    const mouseY = useMotionValue(0);

    // The inner dot gets NO spring so it follows instantly
    const cursorX = mouseX;
    const cursorY = mouseY;

    // Spring for the outer ring (tighter now to reduce aggressive lag)
      ...
      `}</style>
            ...
            <motion.div
                ...
                animate={{
                    ...
                    borderColor: isHovering ? "rgba(220, 160, 40, 0.2)" : "rgba(200, 134, 10, 0.4)",
                    borderWidth: isHovering ? 1 : 1,
                }}
                transition={{ type: "spring", stiffness: 300, damping: 20 }}
            >
                <motion.div
                    className="absolute inset-0 rounded-full border border-[#c8860a] opacity-30"
            ...
            <motion.div
                ...
                animate={{
                    scale: isHovering ? 0 : 1, // Shrink away when hovering (the compass takes over)
                }}
                transition={{ type: "spring", stiffness: 300, damping: 20 }}
            />
        </>
